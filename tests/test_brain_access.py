...
from citadel.services.brain.access import BrainAccess, _NullOracle, _null_degree_one
...
def test_prune_proves_absence_and_skips_the_scan():
    ...
    assert capsule["pruned"] is True and capsule["chunks"] == []
...
def test_emit_publishes_onto_the_bus_idempotently():
    ...
    assert a == b and bus.depth("brain") == 1
...
def test_stats_reports_backend_and_shape():
    ...
    s = brain.stats()
    assert s["bus"] == "memory" and s["adjacency_nodes"] == 1 and s["oracle"]["l0"] == 2
