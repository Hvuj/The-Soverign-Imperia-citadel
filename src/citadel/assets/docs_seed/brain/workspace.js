...
function _renderRepoBody(r, repoName) {
  ...
  const dagDefs  = r.orchestration_defs || [];
  ...
  const dagHTML = dagDefs.length
    ? `<div class="section">
        <div class="section-title">Orchestration definitions <span class="count">${dagDefs.length}</span></div>
...
function _renderModuleBody(m, repoName, modName) {
  ...
  const assets    = m.orchestration_assets || [];
  const schemas   = m.schema_models || [];
  const configmodel  = m.config_models || [];
  ...
  return `
    ...
    ${symbolList(schemas,   "SchemaVal schemas", "orange")}
    ${symbolList(configmodel,  "ConfigModel models", "blue")}
    ${featHTML}${testsHTML}
    <div class="section">
      <div class="ask-chip-row">
...
function _renderFileBody(f, repoName, fileId) {
  ...
  const assets    = f.orchestration_assets || [];
  const checks    = f.schema_checks || [];
  const schemas   = f.schema_models || [];
  const configmodel  = f.config_models || [];
  ...
  return `
    ...
    ${symList(schemas,   "schemaval",  "orange", true)}
    ${symList(configmodel,  "configmodel", "blue",   true)}
