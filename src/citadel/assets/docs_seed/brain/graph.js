...
function render3dGraph() {
  ...
  Graph
    ...
    .onNodeDragEnd((node) => { node.rate = node.x; node.fy = node.y; node.fz = node.z; })
...
async function _loadRepoShard(node) {
  ...
  div.innerHTML =
    `<div class="section-title" style="margin-top:14px">Repo shard â€” ${esc(shard.node_count || nodes.length)} nodes</div>`
    ...
    + (nodes.length > 25 ? `<div class="muted" style="font-size:11px">â€¦and ${nodes.length - 25} more (lazy-loaded on demand)</div>` : "");
